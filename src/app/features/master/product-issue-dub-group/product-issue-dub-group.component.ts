...
@Component({
  selector: 'sa-product-issue-dub-group',
  templateUrl: './product-issue-dub-group.component.html',
  styleUrls: ['./product-issue-dub-group.component.css']
})
export class ProductIssueDubGroupComponent implements OnInit {
public gridOptions;
  constructor() { }
...
@Directive({selector: 'product-issue-sub-group-grid'})
//This is Facade
export class ProdIssueDubGrpDirective extends UpgradeComponent implements OnInit, OnChanges, DoCheck,
OnDestroy {
   @Input() data: {}; 
   @Output() onUpdate: EventEmitter<{}>;
  constructor(elementRef: ElementRef, injector: Injector) {
    super('product-issue-sub-group-grid', elementRef, injector);
  }
...
//This is const, which will be used in .component()
  export const prodIssueDubGrpComponent: angular.IComponentOptions = {
      bindings: {
        data: '<'
      },
      transclude: true,
      controller: ['$scope', '$http', 'uiGridConstants', function($scope, $http, uiGridConstants) {
        ...
        $scope.highlightFilteredHeader = function (row, rowRenderIndex, col, colRenderIndex) {
          ...
          [
            ...
            { name: 'ProdsgDesc',   displayName: 'Product Issue Sub Type',  width: "200", cellTooltip: true, headerCellClass: $scope.highlightFilteredHeader, cellClass: 'left-align' },
            ...
            { name: 'IsAutoClose',  displayName: 'Is Auto Closed',          width: "150", cellTooltip: true, headerCellClass: $scope.highlightFilteredHeader, cellClass: 'text-c' },
            { name: 'IsMailSend',   displayName: 'Is Mail Sent',            width: "150", cellTooltip: true, headerCellClass: $scope.highlightFilteredHeader, cellClass: 'text-c' },            
          ...
          exporterExcelFilename: 'Product Issue Sub Group Master.xlsx',
          ...
          exporterExcelHeader: function (grid, workbook, sheet, docDefinition) {
              ...
              var cols = [];
              cols.push({ value: '' });
              cols.push({ value: 'Bank Master', metadata: { style: formatterId.id } });
        ...
        $scope.ProductIssueSubGroup=function(){
          $http.get(AppModule.baseurl+'Master/GetProductISG?DomainCode&PsuiteCode&ProductCode&ProdissueCode&ProductISCode&isactive&IsMailSend=&IsAutoClose=').then(function (response) {
        ...
        $scope.ProductIssueSubGroup();
          
        $scope.Edit=function(data){
          sessionStorage.productIssueDubGroupdata = JSON.stringify(data);
